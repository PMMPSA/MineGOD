     ...
     public function onEntityDamage(EntityDamageEvent $event) {
        ...
        $damager = $event->getDamager();
           if($damager instanceof Player && $this->isPlayer($damager)) {
              $damager->sendTip("§e༺§l§bＦＬＹ§r§e༻§c Vui Lòng Tắt Fly Để PvP. Cám Ơn!");
        ...
        if(strtolower($cmd->getName()) == "fly") {
            if($sender instanceof Player) {
                if($this->isPlayer($sender)) {
                    ...
                    $sender->sendMessage("§e༺§bＦＬＹ§r§e༻§c Chế Độ Fly Đã Vô Hiệu Hóa");
                    return true;
                }
                else{
                    $this->addPlayer($sender);
                    $sender->setAllowFlight(true);
                    $sender->sendMessage(TextFormat::YELLOW . "§e༺§bＦＬＹ§r§e༻§a Chế Độ Fly Đã Kích Hoạt");
                    return true;
                }
            }
            else{
                $sender->sendMessage(TextFormat::RED . "§e༺§bＦＬＹ§r§e༻§c Chế Độ Fly Đã Vô Hiệu Hóa");
                return true;
            }
        }
    }
